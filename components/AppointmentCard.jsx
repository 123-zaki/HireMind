...
const AppointmentCard = ({ booking, mode, isPast = false }) => {
  ...
  // Destructure necessary data from booking
  const {
    startTime,
    endTime,
    status,
  ...
  const person = mode === "interviewee" ? booking.interviewer : booking.interviewee;

  const creditsLabel = mode === "interviewee" ? `-${creditsCharged} credits charged` : `+${creditsCharged} credits earned`;

  const creditsStyle = mode === "interviewee" ? `border-amber-400/20 bg-amber-400/5 text-amber-400` : `border-green-500/20 bg-green-500/10 text-green-400`;
  ...
  return (
    <>
      <FeedbackModal
        ...
        feedback={feedback}
        intervieweeName={mode === "interviewer" ? booking.interviewee?.name : undefined}
      />
      ...
      <article className='group relative bg-[#0f0f11] border border-white/10 transition-all duration-300 hover:-translate-y-0.5 rounded-2xl bg-linear-to-t from-transparent via-transparent to-amber-300/10 p-7 flex flex-col gap-6'>
              ...
              {mode === "interviewee" && person?.categories?.length > 0 && (
                <div className='flex flex-wrap gap-1 mt-1'>
                  {person.categories.slice(0, 3).map(cat => (
                    <span key={cat} className='text-[10px] px-2 py-0.5 rounded-md border border-amber-400/20 bg-amber-400/5 text-amber-400 leading-tight'>
                      {/* {cat.replace("_", " ")} */}
                      {CATEGORY_LABEL[cat]}
                    </span>
                  ))}
        ...
        <div className='grid grid-cols-3 gap-4'>
          ...
          <div className='flex flex-col gap-1.5'>
            <div className='flex items-center text-stone-600 gap-1.5'>
              <Video size={12} />
              <span className='text-[10px] font-semibold tracking-widest uppercase'>Duration</span>
            </div>
            <p className='text-sm text-stone-300'>{formatDuration(startTime, endTime)}</p>
          </div>
        </div>
        ...
        {feedback?.summary && (
          <div className='rounded-xl border border-white/8 bg-[#141417] px-4 py-3 flex flex-col gap-1.5'>
            <p className='text-stone-600 tracking-widest uppercase font-semibold text-[10px]'>
              AI Feedback.
            </p>
        ...
        {(streamCallId || recordingUrl || feedback) && (
          <div className='flex items-center gap-2 flex-wrap pt-1'>
            {!isPast && streamCallId && isUpcoming && (
              <Button variant='gold' size='sm' className="" asChild>
            ...
            {feedback && (
              <>
                ...
                <Badge variant='outline' className={RATING_STYLES[feedback.overallRating]}>
