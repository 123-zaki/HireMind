...
const AvailabilitySection = ({ initial }) => {
    const [startTime, setStartTime] = useState(
        initial?.startTime ? new Date(initial.startTime).toTimeString().slice(0, 5) : ''
    );
    ...
    const { data, error, loading, fn: saveFn } = useFetch(setAvailability);
        ...
        <section className='bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-7'>
            <div className="flex flex-col justify-between items-start gap-4">
                <div>
                    ...
                    <p className='text-sm text-stone-500 font-light mt-1'>Interviewees can book within this window every day.</p>
                </div>
            ...
            {/* Start and End Times Label and Input */}
            <div className='grid grid-cols-2 gap-4'>
                {/* Start time */}
                <div className='flex flex-col gap-2'>
                    <Label htmlFor="start-time" className={`text-stone-400 text-xs`}>Start Time</Label>
                    <Input
                        type="time"
                        id="start-time"
                        value={startTime}
                ...
                {/* End time */}
                <div className='flex flex-col gap-2'>
                    <Label htmlFor="end-time" className={`text-stone-400 text-xs`}>End Time</Label>
                    <Input
                        type="time"
                        id="end-time"
                        value={endTime}
